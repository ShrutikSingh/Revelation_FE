    ...
    <>
      ...
      <nav
          ...
          {[
            ...
            <button
              ...
              className={`px-3 py-2 text-lg rounded-lg flex items-center gap-2 transition-all duration-300 
                ${activeSection === id ? "bg-red-700 text-white px-4 py-2 font-serif font-medium" : hovered === id ? "bg-red-700 text-white px-4 py-2 font-serif font-medium" : "bg-transparent text-white hover:text-gray-400"}
              `}
            >
              <img src={icon} alt={label} className="w-6 sm:w-8 h-auto filter brightness-200 contrast-200 drop-shadow-md" />
        ...
        <div className="hidden lg:block pr-3">
          {Token !== null ? (
            <div onClick={() => handleNavigation("/profile")} className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-black border-2 border-red-600 flex items-center justify-center cursor-pointer transition-transform hover:scale-110">
        ...
        <div className="lg:hidden flex items-center">
          <button onClick={() => setMenuOpen(true)} className="text-2xl transition-transform hover:scale-110">
            <FaBars />
          </button>
        </div>
      </nav>
      ...
      {/* Mobile Menu */}
      <div className={`fixed top-0 right-0 h-full w-56 bg-gray-900 text-white transform ${menuOpen ? "translate-x-0" : "translate-x-full"} transition-transform duration-300 shadow-lg z-50`}>
        ...
        <div className="flex flex-col items-center gap-4 mt-6">
          {Token !== null ? (
            <div onClick={() => handleNavigation("/profile")} className="w-10 h-10 rounded-full bg-black border-2 border-red-600 flex items-center justify-center cursor-pointer transition-transform hover:scale-110">
              {userData.picture && <img src={userData.picture} alt="Profile" className="w-full h-full rounded-full" />}
            </div>
          ) : (
            // <button onClick={() => handleNavigation("/login")}>Login</button>
          ...
          {[
            ...
            <button key={id} onClick={() => handleNavigation(id)} className="text-lg flex items-center gap-2 w-full px-4 py-2 transition duration-200 hover:bg-red-600">
              <img src={icon} alt={label} className="w-6 sm:w-8 h-auto filter brightness-200 contrast-200 drop-shadow-md" />
              {label}
            </button>
          ))}
